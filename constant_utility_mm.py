...
class ConstantUtilityMM:
	def __init__(self, c_strikes, c_bids, c_asks, p_strikes, p_bids, p_asks):
		self.c_strikes = c_strikes/100
		self.c_bids = c_bids
		self.c_asks = c_asks
		self.p_strikes = p_strikes/100
		self.p_bids = p_bids
		self.p_asks = p_asks
	...
	def mm(self):
		...
		while it < numIt:
			...
			for i in range(0, K.shape[0]):
				...
				if np.sign(call_mm_buy)>=0 and np.absolute(call_mm_buy) < call_bid:
				...
				if np.sign(call_mm_sell)<=0 and np.absolute(call_mm_sell) > call_ask:
					q = q-np.maximum(K-K[i], 0)
