...
if buy_or_sell == 'buy':
	...
	print('Accept {} fraction of the combinatorial bid order on call option of {}{}+{}{} with strike price {} at bid price {}'.format(accept_frac, coeff1, st1, coeff2, st2, strike, price))
	...
	accept_frac = float("{0:.2f}".format(frac[-1, 0]))
	print('Accept {} fraction of the combinatorial ask order on call option of {}{}+{}{} with strike price {} at ask price {}'.format(accept_frac, coeff1, st1, coeff2, st2, strike, price))
	...
	for i in range(0, len(frac[:-1])):
		if frac[i, 0] > 1e-2:
			if i < len(opt1_asks):
				...
				print("Sell {} fraction of call option on {} at strike {} with bid price {}".format(float("{0:.4f}".format(frac[i, 0])), st1, strikes[i], bids[i]))
			else: 
				profit = profit+frac[i, 0]*bids[i]
